...
class IGDB:
    ...
    async def init_key(self, api):
        ...
        response = requests.post(
            f"https://id.twitch.tv/oauth2/token?client_id={id}&client_secret={api.data['secret']}&grant_type=client_credentials"
        ).json()
        token = response["access_token"]
        self.redis.execute(
            "SET", f"igdb-{id}", token, "EX", response["expires_in"]
        )
        return id, token

    def _get_recent_releases(self, id, token):
        cutoff = int(
            arrow.utcnow().shift(weeks=-1).float_timestamp
        )  # TODO: pass from release_checker
